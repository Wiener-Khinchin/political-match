...
export default function ResultPage() {
  ...
  useEffect(() => {
    ...
    if (!bestMatch) {
      ...
      setBestMatch({ id: result.best.id, similarity: Math.round(result.similarity * 100) });
    }

    if (process.env.NEXT_PUBLIC_KAKAO_JS_KEY && !window.Kakao.isInitialized()) {
  ...
  const shareUrl  = `${process.env.NEXT_PUBLIC_SITE_URL}`;      // 대문만
  ...
  return (
    <div className={`min-h-screen ${candidateColorMap[candidate.id]} transition-colors duration-300 flex items-center justify-center`}>
      <div className="w-full max-w-2xl p-6">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="flex flex-col items-center space-y-6">
            ...
            {/* Candidate Election Number and Name */}
            <div className="text-center">
              <p className="text-xl text-gray-600 mb-2">{candidateSymbolMap[candidate.id]}</p>
              <h1 className="text-3xl font-bold">{candidateNameMap[candidate.id]}</h1>
            </div>
            ...
            <div className="w-full pt-6 border-t border-gray-200">
              ...
              <div className="flex justify-center gap-4 mt-4">
                ...
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 md:w-6 md:h-6">
                    <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 5.463 2 9.702c0 2.682 1.76 5.036 4.41 6.39-.19.882-.74 3.22-.85 3.72-.13.56.2.55.37.4.14-.12 2.2-1.74 3.08-2.44.51.07 1.04.11 1.59.11 5.523 0 10-3.463 10-7.702S17.523 2 12 2z" />
                  </svg>
                </button>
              </div>
            </div>
